    ...
    def review(self, dmroot, ID):
        ...
        for iref in intrefs:
            ...
            for idc in IDCOMPILE:
                if re.match(r'{0}'.format(idc), str(iref.attrib['internalRefId'])) is not None:
                    iref.attrib['internalRefTargetType'] = IDCOMPILE[idc]
    ...
    def initiate(self, all_files, dirpath, choice):
        for each in all_files:
            if each.startswith('DMC-') and (each.endswith('.xml') or each.endswith('.XML')):
                ...
                Write_DMC(dmroot1, file, dirpath)
                ...
                if GC[gc] == eref.attrib['referredFragment']:
                    if str(gc).startswith(NameAndCode().name_from_dmcode(eref.find('.//dmCode').attrib)):
                        eref.attrib['referredFragment'] = str(gc).split('||')[
                            1]
        for each in extrefs:
            each.attrib['referredFragment'] = str(each.attrib['referredFragment']).replace('$$', '-')
        return dmroot
...
dirpath = pathlib.PureWindowsPath(sys.argv[1]).as_posix()
os.chdir(dirpath)
...
selection = str(sys.argv[2]).split('_')
Commencement().initiate(all_files, dirpath, selection)

logger.info('Successfully replaced all Idents')
