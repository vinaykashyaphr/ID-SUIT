...
%ISOEntities;'''
        ...
        if all_graphics != []:
            for graphic in all_graphics:
                entity_tif = '<!ENTITY {0} SYSTEM "Graphics/{0}.tif" NDATA tiff>'.format(
                    graphic.attrib['infoEntityIdent'])
                entity_cgm = '<!ENTITY {0} SYSTEM "Graphics/{0}.cgm" NDATA cgm>'.format(
                    graphic.attrib['infoEntityIdent'])
...
class NameAndCode():
    ...
    def name_from_dmcode(self, dmcode_attribute_dictonary):
        adder = ('DMC', dmcode_attribute_dictonary['modelIdentCode'],
                 ...
                 dmcode_attribute_dictonary['subSystemCode'] +
                 dmcode_attribute_dictonary['subSubSystemCode'],
                 dmcode_attribute_dictonary['assyCode'],
                 dmcode_attribute_dictonary['disassyCode'] +
                 dmcode_attribute_dictonary['disassyCodeVariant'],
                 dmcode_attribute_dictonary['infoCode'] +
                 dmcode_attribute_dictonary['infoCodeVariant'],
    ...
    def dmcode_from_name(self, dmc_name):
        ...
        namecompiler = re.compile(
            r'(DMC)-(HON[A-Z0-9]{2,14})-([A-Z0-9]{1,3})-([A-Z0-9]{2,3})-([0-9]{1})([0-9]{1})-([0-9]{2})-([A-Z0-9]{2})([A-Y]{1})-([A-Z0-9]{3})([A-Z]{1})-([A-D]{1})')
        ...
        for each in allfiles:
            if each.startswith('DMC-HON'):
                ...
                if identcode.attrib != dmcname_code.attrib:
                    ...
                    logger.info('Renamed from:: {0} to {1}'.format(
                        NameAndCode().name_from_dmcode(identcode.attrib), dmcname_outside))
...
class LogBuilder():
    ...
    def build_log(self, rootdir: pathlib.Path, name: str, mode: str):
        ...
        filehandler = logging.FileHandler(name, mode, encoding='utf-8')
